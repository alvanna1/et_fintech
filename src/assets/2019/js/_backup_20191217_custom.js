...
$(document).ready(function () {
	...
	if ($('#videoBGStill').length) {
		var desktopVideo = ' <video style="display:none;" id="videoBG" class="videoBackground" playsinline autoplay muted loop><source src="images/header_bg.jpg" type="video/mp4"></video>';
		...
		if ($(window).width() > 1000) {
			...
			$('#zh .colorBoxVideo').colorbox({
				...
				href: function () {
					var videoId = new RegExp('[\\?&]v=([^&#]*)').exec(this.href);
					if (videoId && videoId[1]) {
						return 'http://youtube.com/embed/' + videoId[1] + '?disablekb=1&hl=zh-TW&fs=0&rel=0&autoplay=1';
					}
				}
			});
